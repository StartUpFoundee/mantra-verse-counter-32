  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 dark:from-zinc-900 dark:via-black dark:to-zinc-800 p-4 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-4 md:mb-8 max-w-6xl mx-auto">
        ...
        <h1 className="text-xl md:text-2xl lg:text-3xl xl:text-4xl font-bold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent text-center">
          Active Days
        </h1>
        <Button
          onClick={() => navigate('/time-tracking')}
          className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-blue-400/30 text-xs md:text-sm"
        >
          <Clock className="w-3 h-3 md:w-4 md:h-4 mr-1" />
          <span className="hidden sm:inline">Track Your</span> Time
        </Button>
      </div>

      {/* Stats Cards - Mobile: smaller and in one row, Desktop: keep original */}
      <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-4 lg:gap-6 mb-4 md:mb-8 lg:mb-12 max-w-6xl mx-auto">
        <ModernCard className="p-3 md:p-6 lg:p-8 bg-gradient-to-br from-orange-400/20 to-red-500/20 border-orange-300/30 dark:border-orange-600/30" gradient>
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4">
            <div className="w-8 h-8 md:w-12 md:h-12 lg:w-16 lg:h-16 bg-gradient-to-br from-orange-500 to-red-500 rounded-lg md:rounded-xl flex items-center justify-center shadow-lg">
              <Flame className="w-4 h-4 md:w-6 md:h-6 lg:w-8 lg:h-8 text-white" />
            </div>
            <div className="text-center md:text-left">
              <h3 className="text-xs md:text-lg lg:text-xl font-semibold text-orange-600 dark:text-orange-400 mb-1">Current Streak</h3>
              <div className="text-lg md:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">{streakData.currentStreak}</div>
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">days in a row</p>
            </div>
          </div>
        </ModernCard>

        <ModernCard className="p-3 md:p-6 lg:p-8 bg-gradient-to-br from-emerald-400/20 to-green-500/20 border-emerald-300/30 dark:border-emerald-600/30" gradient>
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4">
            <div className="w-8 h-8 md:w-12 md:h-12 lg:w-16 lg:h-16 bg-gradient-to-br from-emerald-500 to-green-500 rounded-lg md:rounded-xl flex items-center justify-center shadow-lg">
              <TrendingUp className="w-4 h-4 md:w-6 md:h-6 lg:w-8 lg:h-8 text-white" />
            </div>
            <div className="text-center md:text-left">
              <h3 className="text-xs md:text-lg lg:text-xl font-semibold text-emerald-600 dark:text-emerald-400 mb-1">Max Streak</h3>
              <div className="text-lg md:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">{streakData.maxStreak}</div>
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">personal best</p>
            </div>
          </div>
        </ModernCard>

        <ModernCard className="p-3 md:p-6 lg:p-8 bg-gradient-to-br from-purple-400/20 to-indigo-500/20 border-purple-300/30 dark:border-purple-600/30" gradient>
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4">
            <div className="w-8 h-8 md:w-12 md:h-12 lg:w-16 lg:h-16 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-lg md:rounded-xl flex items-center justify-center shadow-lg">
              <Target className="w-4 h-4 md:w-6 md:h-6 lg:w-8 lg:h-8 text-white" />
            </div>
            <div className="text-center md:text-left">
              <h3 className="text-xs md:text-lg lg:text-xl font-semibold text-purple-600 dark:text-purple-400 mb-1">Total Active Days</h3>
              <div className="text-lg md:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">{streakData.totalActiveDays}</div>
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">lifetime practice</p>
            </div>
          </div>
        </ModernCard>
      </div>

      {/* Spiritual Journey Levels - Mobile: smaller, Desktop: keep original */}
      <SpiritualJourneyLevels activityData={activityData} />

      {/* Calendar - Mobile: smaller height, Desktop: keep original */}
      <div className="max-w-6xl mx-auto mb-4 md:mb-8 lg:mb-12">
        <ModernCard className="p-4 md:p-6 lg:p-8 bg-white/80 dark:bg-zinc-800/80 backdrop-blur-xl border-amber-200/50 dark:border-amber-700/50" gradient>
          <div className="mb-4 md:mb-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 md:gap-3">
                <Button
                  onClick={() => navigate('/time-tracking')}
                  className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-purple-400/30 text-xs md:text-sm"
                >
                  <Clock className="w-3 h-3 md:w-4 md:h-4 mr-1" />
                  <span className="hidden sm:inline">Track Your</span> Time
                </Button>
                {yearOptions.length > 1 && (
                  <select 
                    ...
                    className="bg-white dark:bg-zinc-800 border border-amber-200/50 dark:border-amber-700/50 rounded-lg px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm font-medium text-gray-900 dark:text-white"
                  >
          ...
          {/* Mobile: single column, reduced gap. Desktop: keep original grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6">
            {calendarMonths.map((monthData) => (
              <div key={monthData.month} className="bg-white/50 dark:bg-zinc-900/50 rounded-lg p-3 md:p-4">
                <h3 className="text-base md:text-lg font-semibold text-gray-900 dark:text-white mb-2 md:mb-3 text-center">
                  {monthData.name}
                </h3>
                ...
                {/* Days grid - Mobile: smaller cells, Desktop: keep original */}
                <div className="grid grid-cols-7 gap-1">
                  {Array.from({ length: Math.ceil(monthData.days.length / 7) }).map((_, weekIndex) => (
                    monthData.days.slice(weekIndex * 7, (weekIndex + 1) * 7).map((dayData, dayIndex) => {
                      if (!dayData) {
                        return <div key={`empty-${weekIndex}-${dayIndex}`} className="w-6 h-6 md:w-8 md:h-8"></div>;
                      }
                      ...
                      return (
                        <div
                          key={dayData.date}
                          className={`w-6 h-6 md:w-8 md:h-8 rounded-sm cursor-pointer transition-all duration-200 hover:ring-2 hover:ring-amber-400 relative flex items-center justify-center text-xs ${
                        ...
                        >
                          {!dayData.isFuture && dayData.count > 0 && spiritualLevel.icon && (
                            <span className="filter drop-shadow-sm text-[10px] md:text-xs">
                              {spiritualLevel.icon}
                            </span>
                          )}
                        </div>
